...
voteMenu = API.createMenu("VOTA POR EL SIGUIENTE MAPA", 0, 0, 6);
...
voteMenu.OnItemSelect.connect(function(sender, item, index) {
    ...
    API.sendNotification("Has votado por ~b~" + item.Text + "~w~.");
...
API.onServerEventTrigger.connect(function (eventName, args) {
    if (eventName === "startRaceCountdown") {
        ...
        API.sendNotification("3");

        /*var start = API.getGlobalTime();
        while (API.getGlobalTime() - start < 10000) {
            disableControls();
            //countdown.Render2D();
            //API.sleep(0);
            //API.renderScaleform(countdown, 20, 0, 20, 20);
        }*/
        ...
        API.sendNotification("DESTRUYANSE!");
